...
Programs in `semantic-invalid/` parse successfully but are rejected during semantic analysis.

## Invalid

Programs in `invalid/` intentionally fail lexical/parser validation.

## Demo

`demo/demo.vectis` and `demo/run_demo.py` provide the canonical scripted demo. For the graphical demo use `vectis studio`.
