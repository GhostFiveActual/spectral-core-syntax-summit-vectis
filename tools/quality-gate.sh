...
echo
echo "[5/8] Public repository boundary..."
for path in \
    .autonomy \
    artifacts \
    competition \
    ...
    tools/task-gates; do
    if [[ -e "$path" ]]; then
        echo "Internal build/provenance path must not ship in public product tree: $path"
        exit 1
    fi
done
...
bad = []
for root in (Path("src"), Path("tests"), Path("tools"), Path("docs"), Path("examples")):
    if not root.exists():
        continue
    for path in root.rglob("*"):
        if not path.is_file() or path.suffix not in {".py", ".md", ".sh", ".vectis", ".html", ".css", ".js"}:
            continue
        for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
            if line.endswith((" ", "\t")):
                bad.append(f"{path}:{number}: trailing whitespace")
if bad:
    print("\n".join(bad))
    raise SystemExit(1)
PYSPACE
...
echo
echo "[8/8] Symlink boundary scan..."
if find . \
    -path './.git' -prune -o \
    -path './.venv' -prune -o \
    -path './venv' -prune -o \
    ...
    -type l -print \
    | grep -q .; then
    echo "Unexpected symbolic link found in product repository."
    exit 1
fi
